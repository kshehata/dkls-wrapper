...
trait DKGState {
    fn scan_qr(self: Box<Self>, context: &DKGContext, qr_data: QRData) -> (Box<dyn DKGState>, Result<(), GeneralError>);

    fn receive_setup(self: Box<Self>, context: &DKGContext, setup: DKGSetupMessage) -> (Box<dyn DKGState>, Result<(), GeneralError>);
    fn start(self: Box<Self>, context: &DKGContext) -> (Box<dyn DKGState>, Result<(), GeneralError>);
    fn get_setup_message(&self) -> Result<&DKGSetupMessage, GeneralError> {
        Err(GeneralError::InvalidState("Cannot get setup message in current state.".to_string()))
    }
...
impl DKGState for DKGWaitForNetState {
    fn scan_qr(self: Box<Self>, _: &DKGContext, _: QRData) -> (Box<dyn DKGState>, Result<(), GeneralError>) {
        (self, Err(GeneralError::InvalidState("Cannot scan QR in current state.".to_string())))
    }

    fn receive_setup(self: Box<Self>, context: &DKGContext, mut setup: DKGSetupMessage) -> (Box<dyn DKGState>, Result<(), GeneralError>) {
        ...
        setup.parties.push(DeviceInfo::for_sk(context.friendly_name.clone(), &context.sk));
        // send the updated setup message to the network.
        (Box::new(DKGReadyState { setup }), Ok(()))
    }

    fn start(self: Box<Self>, _context: &DKGContext) -> (Box<dyn DKGState>, Result<(), GeneralError>) {
        (self, Err(GeneralError::InvalidState("Cannot start from current state.".to_string())))
    }
...
mod dkg_wait_state_tests {
    ...
    fn gen_sample_context() -> DKGContext {
        ...
        DKGContext { friendly_name: "ourselves".to_string(), sk, setup_if: Arc::new(mock_if) }
    }
    ...
    fn gen_sample_wait_state() -> Box<DKGWaitForNetState> {
        ...
        Box::new(DKGWaitForNetState { qr_data: qr.clone() })
    }
    ...
    fn test_wait_recv_qr_err() {
        ...
        assert_eq!(state.get_state_name(), "Waiting for network".to_string());
        let (new_state, res) = state.scan_qr(&context, qr.clone());
        assert_eq!(new_state.get_state_name(), "Waiting for network".to_string());
    ...
    fn test_wait_recv_setup_ok() {
        ...
        let state = Box::new(DKGWaitForNetState { qr_data: qr.clone() });
        let (res_state, res) = state.receive_setup(&context, setup_msg);
        res.unwrap();
        assert_eq!(res_state.get_state_name(), "Ready".to_string());
...
impl DKGState for DKGReadyState {
    fn get_state_name(&self) -> String {
        "Ready".to_string()
    }
    ...
    fn scan_qr(self: Box<Self>, _: &DKGContext, qr_data: QRData) -> (Box<dyn DKGState>, Result<(), GeneralError>) {
        if qr_data.instance != self.setup.instance
            || qr_data.threshold != self.setup.threshold {
            return (self, Err(GeneralError::InvalidInput("QR data does not match setup data.".to_string())));
        }

        if qr_data.party_id as usize >= self.setup.parties.len() {
            return (self, Err(GeneralError::InvalidInput("Unknown party ID.".to_string())));
        }
        if qr_data.dev_info != self.setup.parties[qr_data.party_id as usize] {
            return (self, Err(GeneralError::InvalidInput("QR device info does not match setup device info.".to_string())));
        }
    ...
    fn receive_setup(self: Box<Self>, _context: &DKGContext, mut setup: DKGSetupMessage) -> (Box<dyn DKGState>, Result<(), GeneralError>) {
        if setup.instance != self.setup.instance
            || setup.threshold != self.setup.threshold {
            return (self, Err(GeneralError::InvalidInput("Received setup message does not match instance or threshold.".to_string())));
        }

        if setup.parties.len() < self.setup.parties.len() {
            return (self, Err(GeneralError::InvalidInput("Received setup message has fewer parties than expected.".to_string())));
        }
        ...
        if setup.parties[0..self.setup.parties.len()] != self.setup.parties {
            return (self, Err(GeneralError::InvalidInput("Received setup message has different parties than expected.".to_string())));
        }
        if setup.start {
...
mod dkg_ready_state_tests {
    ...
    fn make_sample_setup(instance: InstanceId, context: &DKGContext, threshold: u8, n: u8) -> DKGSetupMessage {
        let mut parties: Vec<DeviceInfo> = (0..n - 1).map(
            |i| DeviceInfo::for_sk(format!("node{}", i), &NodeSecretKey::from_entropy())).collect();
        parties.push(DeviceInfo::for_sk(context.friendly_name.clone(), &context.sk));
        DKGSetupMessage {
            instance,
            threshold,
            parties,
    ...
    fn make_sample_data() -> (InstanceId, DKGContext, DKGSetupMessage, Box<DKGReadyState>) {
        let instance = InstanceId::from_bytes(vec![0x01; 32]).unwrap();
        let context = gen_sample_context();
        let setup = make_sample_setup(instance, &context, 2, 3);
        let state = Box::new(DKGReadyState { setup: setup.clone() });
    ...
    fn test_ready_receive_qr_ok() {
        let (instance, context, setup, state) = make_sample_data();
        let qr = make_qr(instance, 2, 0, &setup.parties[0]);
        let (new_state, res) = state.scan_qr(&context, qr);
        assert!(res.is_ok());
        assert_eq!(new_state.get_state_name(), "Ready".to_string());
    ...
    fn test_ready_receive_qr_wrong_instance() {
        let (instance, context, setup, state) = make_sample_data();
        let qr = make_qr(InstanceId::from_bytes(vec![0x02; 32]).unwrap(), 2, 0, &setup.parties[0]);
        let (new_state, res) = state.scan_qr(&context, qr);
        assert_eq!(new_state.get_state_name(), "Ready".to_string());
    ...
    fn test_ready_receive_setup_ok() {
        let (instance, context, mut new_setup, state) = make_sample_data();
        new_setup.parties.push(DeviceInfo::for_sk("new_node".to_string(), &NodeSecretKey::from_entropy()));
        let (state, res) = state.receive_setup(&context, new_setup.clone());
        res.unwrap();
        assert_eq!(state.get_state_name(), "Ready".to_string());
    ...
    fn test_ready_receive_setup_ok_with_verified() {
        let (instance, context, mut new_setup, mut state) = make_sample_data();
    ...
    fn test_ready_receive_setup_wrong_party_info() {
        let (instance, context, mut new_setup, state) = make_sample_data();
        new_setup.parties.pop();
        new_setup.parties.push(DeviceInfo::for_sk("ourselves".to_string(), &NodeSecretKey::from_entropy()));
        let (state, res) = state.receive_setup(&context, new_setup.clone());
        assert_eq!(state.get_state_name(), "Ready".to_string());
        assert!(matches!(res, Err(GeneralError::InvalidInput(_))));
    }
    ...
    fn test_ready_receive_start() {
        ...
        let (state, res) = state.receive_setup(&context, new_setup.clone());
        res.unwrap();
        assert_eq!(state.get_state_name(), "Running".to_string());
    }
...
impl DKGState for DKGRunningState {
    fn get_state_name(&self) -> String {
        "Running".to_string()
    }
    ...
    fn receive_setup(self: Box<Self>, _: &DKGContext, _: DKGSetupMessage) -> (Box<dyn DKGState>, Result<(), GeneralError>) {
        (self, Err(GeneralError::InvalidState("Cannot receive setup in current state.".to_string())))
    }

    fn start(self: Box<Self>, _: &DKGContext) -> (Box<dyn DKGState>, Result<(), GeneralError>) {
        (self, Err(GeneralError::InvalidState("Cannot start from current state.".to_string())))
    }
}
...
pub struct DKGNode {
    ...
    state: Mutex<Option<Box<dyn DKGState>>>,
    context: DKGContext,
    dkg_if: Arc<dyn DKGNetworkInterface>,
}

impl DKGNode {
    pub fn new(name: &str, instance: InstanceId, threshold: u8,
        setup_if: Arc<dyn DKGNetworkInterface>, dkg_if: Arc<dyn DKGNetworkInterface>) -> Self {
    ...
    pub fn get_state_name(&self) -> String {
        self.state.lock().unwrap().as_ref().unwrap().get_state_name()
    }
